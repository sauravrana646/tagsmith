...
- **Later**: hashing/SQLite RAG (Phase 3), `mcp` / FastMCP (Phase 4), FastAPI + Next.js (Phase 5), **hosted SaaS as the customer product** (Phase 6 — [PHASE6.md](PHASE6.md)).
...
**Phase 5 — Product foundation (local / single-user API).** FastAPI wrapping the service layer, web OAuth, dashboard, billing hooks. This is the **operator dogfood** stack. **Customer product is hosted SaaS in Phase 6** ([PHASE6.md](PHASE6.md)).

**Phase 6 — SaaS (the product).** Hosted web app: Sign in with Google, our LLM, always-on labels. Customers do **not** install the CLI. PayPal, $12 Pro, Google verification, Pub/Sub, tenant isolation. See [PHASE6.md](PHASE6.md).
...
3. **Where approvals happen** — CLI in Phase 1; local dashboard in Phase 5; **hosted web app in Phase 6** (customer path).
...
- [ ] **Phase 6 — hosted SaaS (the product)**: web app only, no customer local install (`docs/PHASE6.md`).