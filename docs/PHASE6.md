...
Status: **planned — this is the product.** Local install is **not** a customer
path. Users only use the hosted web app.

Do not start until Phase 4+5 are merged to `main` and dogfooded on **your**
inbox (you may still run the CLI locally as the operator).

**Not legal, tax, or FEMA advice.** Use a CA for GST/inward remittance and a
lawyer for privacy policies and Google verification.
...
**Tagsmith the product** is a hosted mailbox assistant: browser → Sign in with
Google → we label under `AI/` on their Gmail. Always-on. They never install
Python, never create a GCP project, never paste an LLM key.

The git repo’s CLI / local dashboard remains **how we develop and dogfood**.
It is **not** documented or sold as “run this on your laptop.”

“We only apply labels and do not read full mail” is the **product story**. It
does **not** put us outside privacy law: we still access Gmail, send a truncated
body to an LLM, and store decisions **on our servers**.

## Locked product shape

| Assumption | Choice |
|------------|--------|
| Customer surface | **Hosted web app only** |
| Local CLI / BYOK / “install on their PC” | **Dropped** as a product |
| Desktop installer for non-tech | **Dropped** (SaaS in the browser instead) |
| Operator location | **India** |
| Customers | **Worldwide** (self-serve) |
| Company form | **No Pvt Ltd** for v1 (solo / sole prop). Stripe still unlikely. |
| Who owns Google OAuth | **Tagsmith** (our Web client; verification required) |
| Who owns the LLM key | **Tagsmith** (bundled in the service; usage in the plan) |
| Paid SKU | Hosted **Pro** subscription |

Non-technical users get: open site → Google Allow → labeling. That is the whole
onboarding.
...
- **Customer local install** (CLI, their `credentials.json`, their API key)
- Desktop/menu-bar app as the way users run Tagsmith
...
| | Operator (you) | Customer |
|--|----------------|----------|
| How they run it | Repo, CLI, local dashboard to dogfood | **Browser only** on tagsmith host |
| Google | Desktop or web client | **Our** verified Web OAuth |
| LLM | Your `.env` | **Ours**, included in the plan |
| Price | n/a | Free trial / **$12 Pro** |

---

## 2. Pricing

Charge **per mailbox**. One **USD** price worldwide in v1. **No local-CLI SKU.**

| Plan | Price | Limits |
|------|--------|--------|
| Hosted Free | $0 | **50** classified messages / day (try the dashboard) |
| **Pro** | **$12 / month** | Always-on, **2,000** / day, background sync, RAG |
| Pro annual (optional) | **$99 / year** | Same as Pro (~2 months free) |

Do **not** offer lifetime. Do **not** start at $4 or $29+. Raise later if people stay.

Matches `/api/billing/plans` stub (`free` 50/day, `pro` $12 / 2000 day).
